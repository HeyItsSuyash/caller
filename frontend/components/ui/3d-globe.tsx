...
// Custom inline utility to merge class names safely
    ...
    <group ref={groupRef} visible={isVisible}>
      ...
      <group ref={imageGroupRef} position={topPosition}>
        ...
        >
          <div
            className={cn(
              "cursor-pointer overflow-hidden rounded-full bg-neutral-950 border border-white/30 shadow-lg transition-transform duration-200 hover:scale-125 hover:border-emerald-400 hover:ring-1 hover:ring-emerald-400/50",
              hovered && "scale-125 ring-1 ring-white/50 border-emerald-400",
            )}
            style={{
              width: "12px",
              height: "12px",
...
function RotatingGlobe({
  ...
  useFrame(() => {
    if (groupRef.current && config.autoRotateSpeed > 0) {
      groupRef.current.rotation.y += (config.autoRotateSpeed * 0.005);
    }
  });

  return (
    <group ref={groupRef}>
      {/* Base transparent sphere mesh to show slightly transparent white ocean bodies */}
      ...
      {/* Continents sphere overlay (solid white continents mapped with monochrome earth map) */}
      <mesh geometry={geometry}>
        <meshStandardMaterial
          color="#ffffff"
          map={earthTexture}
  ...
  return (
    <Html center>
      <div className="flex shrink-0 flex-col items-center gap-3">
        <span className="inline-block shrink-0 text-sm text-neutral-400 font-display">
          Loading 3D telemetry...
        </span>
      </div>
    </Html>
  );
...
const defaultConfig: Required<Globe3DConfig> = {
  radius: 2,
  globeColor: "#ffffff",
  textureUrl: "https://unpkg.com/three-globe@2.31.0/example/img/earth-dark.jpg",
